 ...
 *
 *  Modificado: 20 Jun 2017 por jgarcia1112@gmail.com
 *
...
static void* logic_channel_addrs[] = { 0xE7E7E7E7E7, 0xC2C2C2C2C2, 0xC2C2C2C2C3, 0xC2C2C2C2C4, 0xC2C2C2C2C5, 0xC2C2C2C2C6 };
...
void nRF24L01P::startTx(uint8_t logicChannel) {
    ...
    RQST_TO_DEVICE(W_REGISTER | TX_ADDR, (uint8_t*)logic_channel_addrs[(logicChannel >> 1)]);
    
    //  asigna la direccion al puerto de recepcion para recibir confirmacion (autoACK)

    RQST_TO_DEVICE(W_REGISTER | RX_ADDR_P0, (uint8_t*)logic_channel_addrs[(logicChannel >> 1)]);
...
void nRF24L01P::loadAddress() {
    ...
    RQST_TO_DEVICE(W_REGISTER | RX_ADDR_P0, (uint8_t*)logic_channel_addrs[0]);

    RQST_TO_DEVICE(W_REGISTER | RX_ADDR_P1, (uint8_t*)logic_channel_addrs[1]);

    RQST_TO_DEVICE(W_REGISTER | RX_ADDR_P2, (uint8_t*)logic_channel_addrs[2]);

    RQST_TO_DEVICE(W_REGISTER | RX_ADDR_P3, (uint8_t*)logic_channel_addrs[3]);

    RQST_TO_DEVICE(W_REGISTER | RX_ADDR_P4, (uint8_t*)logic_channel_addrs[4]);

    RQST_TO_DEVICE(W_REGISTER | RX_ADDR_P5, (uint8_t*)logic_channel_addrs[5]);
    
    // establece la direccion del transmisor (PTX) igual al canal 1 (RX_ADDR_P0)

    RQST_TO_DEVICE(W_REGISTER | TX_ADDR, (uint8_t*)logic_channel_addrs[0]);
 ...
 *
 */
void nRF24L01P::spiWrite(uint8_t cmd, const int8_t* pBuffer, uint8_t lenBuffer) {
