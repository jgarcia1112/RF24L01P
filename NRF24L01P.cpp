...
void nRF24L01P::startTx(uint8_t logicChannel) {
    ...
    //  carga y establece la direccion del canal en el que se va a transmitir

    uint8_t len = sizeof(LOGIC_CHANNEL_ADDR1);
    
    uint8_t* pLogicAddr;
 
    switch (logicChannel) {
        case CHANNEL1:
            pLogicAddr = LOGIC_CHANNEL_ADDR1;
            break;
            
        case CHANNEL2:
            pLogicAddr = LOGIC_CHANNEL_ADDR2;
            break;
        
        case CHANNEL3:
            pLogicAddr = LOGIC_CHANNEL_ADDR3;
            break;
        
        case CHANNEL4:
            pLogicAddr = LOGIC_CHANNEL_ADDR4;
            break;
        
        case CHANNEL5:
            pLogicAddr = LOGIC_CHANNEL_ADDR5;
            break;
        
        case CHANNEL6:
            pLogicAddr = LOGIC_CHANNEL_ADDR6;
            break;
    ...
    spiWrite(W_REGISTER | TX_ADDR, pLogicAddr, len);
    
    //  asigna la direccion al puerto de recepcion para recibir confirmacion (autoACK)
    
    spiWrite(W_REGISTER | RX_ADDR_P0, pLogicAddr, len);
    
    //  establece el dispositivo en modo PTX
...
void nRF24L01P::loadAddress() {
    ...
    spiWrite(W_REGISTER | RX_ADDR_P0, LOGIC_CHANNEL_ADDR1, len);
    
    // establece la direccion del PRX en el puerto 2 (pipe 1)
    
    spiWrite(W_REGISTER | RX_ADDR_P1, LOGIC_CHANNEL_ADDR2, len);
    
    // establece la direccion del PRX en el puerto 3 (pipe 2)
    
    spiWrite(W_REGISTER | RX_ADDR_P2, LOGIC_CHANNEL_ADDR3[len-1]);
    
    // establece la direccion del PRX en el puerto 4 (pipe 3)
    
    spiWrite(W_REGISTER | RX_ADDR_P3, LOGIC_CHANNEL_ADDR4[len-1]);
    
    // establece la direccion del PRX en el puerto 5 (pipe 4)
    
    spiWrite(W_REGISTER | RX_ADDR_P4, LOGIC_CHANNEL_ADDR5[len-1]);
    
    // establece la direccion del PRX en el puerto 6 (pipe 5)
    
    spiWrite(W_REGISTER | RX_ADDR_P5, LOGIC_CHANNEL_ADDR6[len-1]);
    
    // establece la direccion del PTX
    
    spiWrite(W_REGISTER | TX_ADDR, LOGIC_CHANNEL_ADDR1, len);
