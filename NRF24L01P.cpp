 ...
 *
 *  Modificado: 23 Jun 2017 por jgarcia1112@gmail.com
 *
...
void nRF24L01P::startRx() {
    ...
    // carga y establece las direcciones de los canales logicos por default
 ...
 *
 */
void nRF24L01P::startTx(uint8_t channel) {

    //  establece el ancho de la direccion del dispositivo

    setAddrWidth(ADDRESS_WIDTH-2);

    //  asigna la direccion al puerto de transmision

    spiWrite(W_REGISTER | TX_ADDR, logic_channel_addrs, ADDRESS_WIDTH, (channel * ADDRESS_WIDTH) + ADDRESS_WIDTH);

    //  asigna la direccion al puerto de recepcion para recibir confirmacion (autoACK)

    spiWrite(W_REGISTER | RX_ADDR_P0, logic_channel_addrs, ADDRESS_WIDTH, (channel * ADDRESS_WIDTH) + ADDRESS_WIDTH);
...
void nRF24L01P::loadAddress() {
    ...
    // establece las direcciones en los puertos de recepcion (PRX)

    spiWrite(W_REGISTER | RX_ADDR_P0, logic_channel_addrs, ADDRESS_WIDTH, (CHANNEL1 * ADDRESS_WIDTH) + ADDRESS_WIDTH);

    spiWrite(W_REGISTER | RX_ADDR_P1, logic_channel_addrs, ADDRESS_WIDTH, (CHANNEL2 * ADDRESS_WIDTH) + ADDRESS_WIDTH);

    spiWrite(W_REGISTER | RX_ADDR_P2, logic_channel_addrs, 1, (CHANNEL3 * ADDRESS_WIDTH) + ADDRESS_WIDTH);

    spiWrite(W_REGISTER | RX_ADDR_P3, logic_channel_addrs, 1, (CHANNEL4 * ADDRESS_WIDTH) + ADDRESS_WIDTH);

    spiWrite(W_REGISTER | RX_ADDR_P4, logic_channel_addrs, 1, (CHANNEL5 * ADDRESS_WIDTH) + ADDRESS_WIDTH);

    spiWrite(W_REGISTER | RX_ADDR_P5, logic_channel_addrs, 1, (CHANNEL6 * ADDRESS_WIDTH) + ADDRESS_WIDTH);

    // establece la direccion del transmisor (PTX) igual al canal 1 (RX_ADDR_P0)
    
    //spiWrite(W_REGISTER | TX_ADDR, logic_channel_addrs, ADDRESS_WIDTH, (CHANNEL1 * ADDRESS_WIDTH) + ADDRESS_WIDTH);
