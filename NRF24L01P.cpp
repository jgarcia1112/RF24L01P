 ...
 *
 *  Modificado: 21 Jun 2017 por jgarcia1112@gmail.com
 *
...
/*
 *
 *  direcciones fisicas de los canales logicos
 *
 */
static uint8_t logic_channel_addrs[] {
    
        0xE7, 0xE7, 0xE7, 0xE7, 0xE7,   // RX_ADDR_P0 - CHANNEL1 //
    
        0xC2, 0xC2, 0xC2, 0xC2, 0xC2,   // RX_ADDR_P1 - CHANNEL2 //
    
        0xC2, 0xC2, 0xC2, 0xC2, 0xC3,   // RX_ADDR_P2 - CHANNEL3 //
    
        0xC2, 0xC2, 0xC2, 0xC2, 0xC4,   // RX_ADDR_P3 - CHANNEL4 //
    
        0xC2, 0xC2, 0xC2, 0xC2, 0xC5,   // RX_ADDR_P4 - CHANNEL5 //
    
        0xC2, 0xC2, 0xC2, 0xC2, 0xC6    // RX_ADDR_P5 - CHANNEL6 //
};
 ...
 *
 *  con auto ACK habilitado (EN_AA), Dynamic Payload Length (DYNPD) en todos los puertos (pipes),
 *
 *  y habilitados todos los canales para la escucha (EN_RXADDR).
 *
 */
void nRF24L01P::startRx() {
    
    // carga y establece las direcciones de los canales logicos
 ...
 *
 *  utilizar stop() para detener toda la actividad del modulo y dejarlo en reposo.
 *
 */
void nRF24L01P::startTx(uint8_t logicChannel) {

    //  establece el ancho de la direccion del dispositivo

    setAddrWidth(DEFAULT_ADDR_WIDTH-2);

    //  asigna la direccion al puerto de transmision

    spiWrite(W_REGISTER | TX_ADDR, logic_channel_addrs, DEFAULT_ADDR_WIDTH, (logicChannel >> 1) * DEFAULT_ADDR_WIDTH);

    //  asigna la direccion al puerto de recepcion para recibir confirmacion (autoACK)

    spiWrite(W_REGISTER | RX_ADDR_P0, logic_channel_addrs, DEFAULT_ADDR_WIDTH, (logicChannel >> 1) * DEFAULT_ADDR_WIDTH);

    //  establece el dispositivo en modo PTX (transmision)
 ...
 *
 */
void nRF24L01P::loadAddress() {

    //  establece el ancho de la direccion del dispositivo

    setAddrWidth(DEFAULT_ADDR_WIDTH-2);
    
    // establece la direccion del transmisor (PTX) igual al canal 1 (RX_ADDR_P0)
    
    spiWrite(W_REGISTER | TX_ADDR, logic_channel_addrs, DEFAULT_ADDR_WIDTH, (CHANNEL1 >> 1) * DEFAULT_ADDR_WIDTH);

    // establece las direcciones en los puertos de recepcion (PRX) y transmision (PTX)
    
    spiWrite(W_REGISTER | RX_ADDR_P0, logic_channel_addrs, DEFAULT_ADDR_WIDTH, (CHANNEL1 >> 1) * DEFAULT_ADDR_WIDTH);

    spiWrite(W_REGISTER | RX_ADDR_P1, logic_channel_addrs, DEFAULT_ADDR_WIDTH, (CHANNEL2 >> 1) * DEFAULT_ADDR_WIDTH);

    spiWrite(W_REGISTER | RX_ADDR_P2, logic_channel_addrs, 1, ((CHANNEL3 >> 1) * DEFAULT_ADDR_WIDTH) + DEFAULT_ADDR_WIDTH - 1);

    spiWrite(W_REGISTER | RX_ADDR_P3, logic_channel_addrs, 1, ((CHANNEL4 >> 1) * DEFAULT_ADDR_WIDTH) + DEFAULT_ADDR_WIDTH - 1);

    spiWrite(W_REGISTER | RX_ADDR_P4, logic_channel_addrs, 1, ((CHANNEL5 >> 1) * DEFAULT_ADDR_WIDTH) + DEFAULT_ADDR_WIDTH - 1);

    spiWrite(W_REGISTER | RX_ADDR_P5, logic_channel_addrs, 1, ((CHANNEL6 >> 1) * DEFAULT_ADDR_WIDTH) + DEFAULT_ADDR_WIDTH - 1);
...
void nRF24L01P::spiWrite(uint8_t cmd, const int8_t* pBuffer, uint8_t lenBuffer, uint8_t start) {
    ...
    while (lenBuffer-- > 0)
        ...
        SPI.transfer((uint8_t)pBuffer[start + lenBuffer] );
 ...
 *
 */
uint8_t nRF24L01P::dataSent() { return DATA_IS_SENT(); }
 ...
 *
 */
uint8_t nRF24L01P::lostConnection() { return LOST_CONNECTION(); }
 ...
 *
 */
void nRF24L01P::enableLiveStream() { SET_CHIP_ENABLE(); }
 ...
 *
 */
void nRF24L01P::disableLiveStream() { RST_CHIP_ENABLE(); }
...
void nRF24L01P::sendData(void* buffer, uint8_t lenBuffer) {
    ...
    //if (IS_PRX_MODE() == true) return;
        ...
        spiWrite(W_TX_PAYLOAD, pTxData, lenBuffer);
...
void nRF24L01P::sendDataFast(void* buffer, uint8_t lenBuffer) {
    ...
    //if (IS_PRX_MODE() == true) return;
        ...
        spiWrite(W_TX_PAYLOAD_NO_ACK, pTxData, lenBuffer);
...
void nRF24L01P::attachToSender(void* buffer, uint8_t lenBuffer, uint8_t senderPortPTX) {
    ...
    //if (IS_PRX_MODE() == false) return;
        ...
        spiWrite(W_ACK_PAYLOAD | senderPortPTX, pTxData, lenBuffer);
 ...
 *
 */
uint8_t nRF24L01P::getDeviceMode() { return IS_PRX_MODE(); }
 ...
 *
 */
void nRF24L01P::setCrcMode(uint8_t mode) { SET_CRC(mode); }
 ...
 *
 */
uint8_t nRF24L01P::getCrcMode() { return (GET_DEVICE_CONFIG() & 0x0C); }
 ...
 *
 */
uint8_t nRF24L01P::getDataRate() { return (GET_RF_SETUP() & 0x28); }
 ...
 *
 */
uint8_t nRF24L01P::getOutputPower() { return (GET_RF_SETUP() & 0x06); }
